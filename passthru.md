...
edite `/etc/mkinitcpio.conf` and add `vfio vfio_pci vfio_iommu_type1` inside the `()` of `Modules()` then ran and add replace `HOOKS()` for this;

    HOOKS=(base udev autodetect microcode modconf keyboard keymap block filesystems fsck)

create `/etc/modprobe.d/vfio.conf` with the following content;
...
now create the virtual  machine and add the gpu as hardware and `ich9` as audio.
